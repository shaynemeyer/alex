...
graph TB
    ...
    ResearcherLambda[fa:fa-bolt Lambda<br/>alex-researcher<br/>AI Agent Service]
    ...
    %% Connections
    ResearcherLambda -->|Store Research| APIGW
    ResearcherLambda -->|Generate| Bedrock
    APIGW -->|Invoke| Lambda
    
    EventBridge -->|Every 2hrs| SchedulerLambda
    SchedulerLambda -->|Invoke| ResearcherLambda
    
    Lambda -->|Get Embeddings| SageMaker
    Lambda -->|Store Vectors| S3Vectors
    
    ResearcherLambda -.->|Pull Image| ECR
    ...
    class APIGW,Lambda,ResearcherLambda,SageMaker,ECR,SchedulerLambda aws
    class Bedrock ai
...
- **alex-scheduler**: Triggers automated research
  ...
  - Memory: 128MB
  - Timeout: 150 seconds
- **alex-researcher**: AI research agent (container image)
  - Runtime: Container (Python 3.12 + Node.js 22)
  - Memory: 2048MB
  - Timeout: 300 seconds
  - Features: Playwright MCP for web browsing, Function URL for direct invocation
...
| Lambda (researcher) | ~$2-5 | Container image, pay per invocation |
...
- **Compute**: Lambda (including container image for researcher)
- **AI/ML**: SageMaker, AWS Bedrock
- **Storage**: S3 Vectors
- **API**: API Gateway
- **Languages**: Python 3.12
- **Container**: Podman / Docker (for building researcher image)
