...
async def create_test_user(user_num: int, num_accounts: int, num_positions: int):
    ...
    for acct_num in range(1, accounts_to_create + 1):
        ...
        if num_positions > 0 and accounts_to_create > 0:
            positions_for_account = num_positions // accounts_to_create + (
                1 if acct_num <= (num_positions % accounts_to_create) else 0
            )
...
async def send_job_to_sqs(job_id: str):
    """Send a job to SQS"""
    sqs = boto3.client("sqs", region_name=os.getenv("DEFAULT_AWS_REGION", "us-west-2"))
    ...
    # Send message
    message = {"job_id": job_id, "timestamp": datetime.now().isoformat()}
...
async def run_scale_test():
    ...
    test_configs = [
        {
            ...
            "num_accounts": 1,
            "num_positions": 0,
        },  # Empty portfolio (single account)
        {
            "user_num": 2,
            "num_accounts": 1,
            "num_positions": 3,
        },  # Small portfolio (single account)
        {
            "user_num": 3,
            "num_accounts": 2,
            "num_positions": 5,
        },  # Medium portfolio (multiple accounts)
        {
            "user_num": 4,
            "num_accounts": 3,
            "num_positions": 10,
        },  # Large portfolio (multiple accounts)
        {
            "user_num": 5,
            "num_accounts": 2,
            "num_positions": 7,
        },  # Mixed portfolio (multiple accounts)
    ]
    ...
    for config in test_configs:
        ...
        print(
            f"  User {config['user_num']}: {user_data['num_accounts']} accounts, {user_data['num_positions']} positions"
        )
    ...
    for i, result in enumerate(results):
        ...
        if status == "completed":
            ...
            failed += 1
            print(
                f"❌ User {user['user_num']}: Failed - {result.get('error', 'Unknown')}"
            )
        else:
            timed_out += 1
    ...
    for user in all_users:
        ...
        if job["status"] == "completed":
            ...
            print(
                f"  User {user['user_num']}: Report {report_size:,} chars, {num_charts} charts, Retirement: {has_retirement}"
            )
