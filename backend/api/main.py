...
sqs_client = boto3.client(
    "sqs", region_name=os.getenv("DEFAULT_AWS_REGION", "us-west-2")
)
        ...
        if not instrument:
            ...
            new_instrument = InstrumentCreate(
                ...
                current_price=Decimal(
                    "0.00"
                ),  # Price will be updated by background processes
        ...
        return [
            {
                ...
                "current_price": (
                    float(inst["current_price"]) if inst.get("current_price") else None
                ),
            }
